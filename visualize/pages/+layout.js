...
async function addYearLinksToManifest(pagesManifest, query) {
    const yearNode = pagesManifest?.children?.year;
    if (!yearNode) return;
...
async function addSportLinksToManifest(pagesManifest, query) {
    ...
    const sports = await query(
        `
            select sport_type, sport_slug, activity_count
            from strava.src_strava__kpis
