    ...
    <div class="flex flex-col h-full pb-4">
      ...
      >
        ...
        {#each firstLevelFiles as file}
          ...
          {/if}
        {/each}
        <!-- CUSTOM SLOT: Content at bottom of mobile scrollable area -->
        <div class="pb-6 border-t border-base-200 pt-4 mt-4">
...
<aside class="w-48 flex-none {sidebarFrontMatter === 'hide' ? 'hidden' : 'hidden md:flex'}">
  {#if !mobileSidebarOpen}
    ...
    <div
        class="hidden: md:block fixed w-48 top-20 overflow-y-auto flex-1 text-sm pretty-scrollbar"
    ...
    <!-- Fixed content at bottom of sidebar (always visible) -->
    <div class="hidden md:block fixed w-48 bottom-0 bg-base-100 border-t border-base-200 pt-4 pb-4 px-2">
      <slot name="desktop-bottom"/>
      {#if builtWithEvidence}
        <div class="text-xs py-2 text-center">
          <a
