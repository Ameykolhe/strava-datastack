"""Custom pagination with rate limiting integration."""
...
class RateLimitedPaginator(PageNumberPaginator):
    """
    Page number paginator with integrated rate limiting.

    Extends dlt's PageNumberPaginator to enforce rate limits
    and track requests per resource.
    """
    ...
    def update_request(self, request: Request) -> None:
        """
        Update request with pagination parameters and enforce rate limit.
        ...
        logger.debug(
            ...
            f"resource_requests={self._resource_requests}"
        )
